...
#include "presto_cpp/presto_protocol/iceberg_connector_protocol.h"
#include "presto_cpp/presto_protocol/hive_connector_protocol.h"
#include "presto_cpp/presto_protocol/tpch_connector_protocol.h"
...
toHiveSortingColumns(const protocol::List<protocol::hive::SortingColumn>& sortedBy) {
  std::vector<std::shared_ptr<const velox::connector::hive::HiveSortingColumn>>
      sortingColumns;
...
toVeloxHiveColumnHandle(
    ...
    const TypeParser& typeParser) {
  auto* hiveColumn = dynamic_cast<const protocol::hive::HiveColumnHandle*>(column);
...
HivePrestoToVeloxConnector::toVeloxSplit(
  ...
  auto hiveSplit = dynamic_cast<const protocol::hive::HiveSplit*>(connectorSplit);
  VELOX_CHECK_NOT_NULL(
...
HivePrestoToVeloxConnector::createVeloxPartitionFunctionSpec(
  ...
  auto hivePartitioningHandle =
      dynamic_cast<const protocol::hive::HivePartitioningHandle*>(partitioningHandle);
  VELOX_CHECK_NOT_NULL(
...
IcebergPrestoToVeloxConnector::toVeloxTableHandle(
  ...
  auto icebergLayout =
      std::dynamic_pointer_cast<const protocol::iceberg::IcebergTableLayoutHandle>(
          tableHandle.connectorTableLayout);
  VELOX_CHECK_NOT_NULL(
      icebergLayout,
      "Unexpected layout type {}",
...
TpchPrestoToVeloxConnector::toVeloxSplit(
    const protocol::ConnectorId& catalogId,
    const protocol::ConnectorSplit* const connectorSplit) const {
  auto tpchSplit = dynamic_cast<const protocol::tpch::TpchSplit*>(connectorSplit);
  VELOX_CHECK_NOT_NULL(
...
TpchPrestoToVeloxConnector::toVeloxColumnHandle(
    const protocol::ColumnHandle* column,
    const TypeParser& typeParser) const {
  auto tpchColumn = dynamic_cast<const protocol::tpch::TpchColumnHandle*>(column);
  VELOX_CHECK_NOT_NULL(
