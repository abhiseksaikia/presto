...
TEST_F(PrestoToVeloxSplitTest, customSplitInfo) {
  auto scheduledSplit = makeHiveScheduledSplit();
  auto& hiveSplit =
      static_cast<protocol::hive::HiveSplit&>(*scheduledSplit.split.connectorSplit);
...
TEST_F(PrestoToVeloxSplitTest, serdeParameters) {
  auto scheduledSplit = makeHiveScheduledSplit();
  auto& hiveSplit =
      dynamic_cast<protocol::hive::HiveSplit&>(*scheduledSplit.split.connectorSplit);
...
TEST_F(PrestoToVeloxSplitTest, bucketConversion) {
  ...
  hiveSplit.bucketConversion = std::make_shared<protocol::hive::BucketConversion>();
