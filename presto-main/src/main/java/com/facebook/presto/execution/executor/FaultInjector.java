...
//        }
        ...
        long bufferedPages = taskExecutor.getTaskList().stream().mapToLong(taskHandle -> taskHandle.getOutputBuffer().get().getInfo().getTotalBufferedPages()).sum();
        if (bufferedPages > 1 && !isShutDown.get()) {
            ...
            try {
                Files.write("http://127.0.0.1:" + port.getAsInt(), shutdownFile, UTF_8);
            }
            catch (IOException e) {
                e.printStackTrace();
            }
